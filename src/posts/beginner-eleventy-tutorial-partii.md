...
When you think of Git, you might think of BitBucket, GitHub, or GitLab. Technically, all those products offer **Git hosting as a service;** providing storage to the code somewhere where many people can access—this is the underpinnings of what it means for the version controlling to be distributed. Git hosts also often a whole _host_ of other benefits, like helper tools (a desktop GUI or web interface that visualises what’s happening in the command line, for instance), allow you to create profiles, and find new projects and collaborators.
...
2. Navigate to the folder you want (use `npm` to go into a folder; use `cd ..` to back out of a folder). (Note that depending on your operating system, the back slashes (\\) could be forward slashes instead (/).
